...
def main() -> int:
    ...
    # выкидываем мусорные строки без подразделения (хвостовые/пустые из исходных файлов —
    # у них заполнен только «Обьем работ, м3»=0, объём при этом =0, данные не теряются);
    # в реестре пустых строк быть не должно
    _podr = full["Подразделение"].astype(str).str.strip()
    _bad = _podr.isin(["", "nan", "Итог", "Итого", "итог", "итого"])
    full = full[full["Подразделение"].notna() & ~_bad].reset_index(drop=True)
    # нормализуем «Передел»: схлопываем лишние/неразрывные пробелы → УНИКАЛЬНЫЕ имена
    # (в источниках встречается «Транспортировка песков», «...песков », «...песков   »)
    full["Передел"] = full["Передел"].apply(
